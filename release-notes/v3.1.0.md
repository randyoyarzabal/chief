...
- `chief.ssl.view_cert` → **Now:** `chief.ssl_view-cert`
- `chief.oc.login` → **Now:** `chief.oc_login`
- `chief.aws.set_role` → **Now:** `chief.aws_set-role`
...
- SSL Plugin: `chief.ssl_create_ca` → `chief.ssl_create-ca`
- SSH Plugin: `chief.ssh_rm_host` → `chief.ssh_rm-host`  
- Python Plugin: `chief.python_create_ve` → `chief.python_create-ve`
- OpenShift Plugin: `chief.oc_get_all_objects` → `chief.oc_get-all-objects`
- Git Plugin: `chief.git_rename_branch` → `chief.git_rename-branch`
- Etc Plugin: `chief.etc_create_bootusb` → `chief.etc_create-bootusb`
- AWS Plugin: `chief.aws_set_role` → `chief.aws_set-role`

**Migration:** Update any scripts, aliases, or documentation that reference the old underscore notation. The new convention matches functions like `chief.vault_file-edit` for consistency across all plugins.
