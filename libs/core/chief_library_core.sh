...
function __chief.hints_text() {
  ...
  if ${CHIEF_CFG_HINTS}; then
    ...
    if [[ ${CHIEF_CFG_PLUGINS_TYPE} == "remote" ]] && ! ${CHIEF_CFG_PLUGINS_GIT_AUTOUPDATE}; then   
      echo -e "${CHIEF_COLOR_YELLOW}CHIEF_CFG_PLUGINS_GIT_AUTOUPDATE=false Run ${CHIEF_COLOR_CYAN}chief.plugins_update${CHIEF_COLOR_YELLOW} to update/load plugins.${CHIEF_NO_COLOR}"
    fi
